...
Individuo* individuo_gera_mutante(Individuo* x1, Individuo* x2, Individuo* x3, int num_veiculos, int num_cidades) {
	...
	int num_cidades_possiveis = num_cidades -1,
	    index_cidade_selecionada = 0;
	    
	int index;
	for(index = 0; index < num_cidades; index++)
		cidades_possiveis[index] = index;
	    
	Individuo* mutante = individuo_clona(x1, num_cidades, num_veiculos);
	...
	int qtd_componentes_perturbadas_atuais = 0,
	    qtd_componentes_perturbadas = (num_cidades/2.0) * F;
	do {
		...
		if(fim_rotas[rota_mutante] < index_mutante +1){ /* A rota nao possui a quantidade de cidades necessária. Entao a cidade será inserida ao fim da rota. */
			individuo_remove_cidade(mutante, cidade_escolhida, 0);
